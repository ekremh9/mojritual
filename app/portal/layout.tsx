...
export default async function PortalLayout({ children }: { children: ReactNode }) {
  ...
  // Isti obrazac kao Header/KorisnickiMeni na shop stranicama — dohvat u
  // server komponenti (layout), proslijeđen kao prop, ne poseban klijentski
  // poziv. Računa se prije `!pristup` grane jer PortalHeader (i time
  // KorisnickiMeni) renderuje u OBA slučaja.
  const brojNeprocitanih = await getUnreadCount(session.user.id);

  if (!pristup) {
    return (
      <div className="flex min-h-full flex-1 flex-col bg-[#F2F5ED]">
        <PortalHeader user={session.user} unreadCount={brojNeprocitanih} />
  ...
  return (
    <BrandProvider brand={brand}>
      <div className="flex min-h-full flex-1 flex-col bg-[#F2F5ED]">
        <PortalHeader
          user={session.user}
          unreadCount={brojNeprocitanih}
          brand={{ naziv: brand.naziv, status: brand.status }}
        />
        ...
        <div className="mx-auto flex w-full max-w-6xl flex-1 flex-col gap-6 px-4 py-6 sm:flex-row sm:gap-10 sm:px-6 sm:py-8">
          <aside className="sm:w-52 sm:shrink-0">
            <PortalNav unreadCount={brojNeprocitanih} />
          </aside>
