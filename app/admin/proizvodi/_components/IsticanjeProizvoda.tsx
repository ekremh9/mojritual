...
import { toggleFeaturedAction } from '@/lib/domain/admin-actions';
...
/**
 * Ručno upravljanje `products.istaknut` — nezavisno od odobrenja statusa
 * proizvoda (radi na svakom statusu), i nezavisno od `istaknutZahtjev`
 * (samo namjera brenda, admin bira šta se stvarno primjenjuje).
 */
export function IsticanjeProizvoda({
  productId,
  istaknutZahtjev,
  istaknut: istaknutPocetno,
  odobren,
  ...
  async function promijeni() {
    ...
    try {
      const rezultat = await toggleFeaturedAction(productId, novoStanje);
    ...
    <section className="flex flex-col gap-3 rounded-2xl border border-[#1C2B22]/10 bg-white p-4">
      ...
      <div className="flex flex-col gap-1.5 text-sm">
        <div className="flex justify-between gap-4">
          <span className="text-[#1C2B22]/60">{poruke.zahtjev}</span>
          <span className="text-[#1C2B22]">{istaknutZahtjev ? poruke.zahtjevDa : poruke.zahtjevNe}</span>
        </div>
        <div className="flex justify-between gap-4">
