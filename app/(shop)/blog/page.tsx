  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-12">
      <div className="flex flex-col gap-2">
        <h1 className="font-bodoni text-2xl font-semibold text-ritual-charcoal sm:text-3xl">{bs.blog.naslov}</h1>
        <p className="text-sm text-ritual-charcoal/60">{bs.blog.podnaslov}</p>
      </div>

      {clanci.length === 0 ? (
        <div className="mt-8 flex flex-col items-center gap-4 rounded-2xl bg-ritual-green/30 px-6 py-16 text-center">
          <p className="max-w-md text-base text-ritual-charcoal/70">{bs.blog.prazno}</p>
        </div>
      ) : (
