...
export default async function NalogPage() {
  ...
  return (
    <div className="mx-auto flex max-w-3xl flex-col gap-8 px-4 py-8 sm:px-6 sm:py-12">
      <h1 className="text-2xl font-semibold text-[#1C2B22] sm:text-3xl">{bs.nalog.naslov}</h1>

      <section className="flex flex-col gap-4 rounded-2xl border border-[#1C2B22]/10 bg-white p-5">
        <h2 className="text-lg font-semibold text-[#1C2B22]">{bs.nalog.podaci.naslov}</h2>

        <div className="flex flex-col gap-1.5">
          <span className="text-sm font-medium text-[#1C2B22]">{bs.nalog.podaci.email}</span>
          <p className="w-full rounded-xl border border-[#1C2B22]/10 bg-[#F2F5ED] px-4 py-2.5 text-sm text-[#1C2B22]/70">
            {session.user.email}
          </p>
          <p className="text-xs text-[#1C2B22]/60">{bs.nalog.podaci.emailNapomena}</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <Link
          href="/nalog/narudzbe"
          className="flex items-center gap-3 rounded-2xl border border-[#1C2B22]/10 bg-white p-5 transition-shadow hover:shadow-md"
        >
          <ClipboardList className="h-5 w-5 shrink-0 text-[#16332A]" aria-hidden="true" />
        ...
        <Link
          ...
          className="flex items-center gap-3 rounded-2xl border border-[#1C2B22]/10 bg-white p-5 transition-shadow hover:shadow-md"
        >
          <Sparkles className="h-5 w-5 shrink-0 text-[#16332A]" aria-hidden="true" />
          <span className="text-sm font-medium text-[#1C2B22]">
